...
/**
  存储的是job的详细信息，包括：[DESCRIPTION]描述，
  [IS_DURABLE]是否持久化，[JOB_DATA]持久化对象等基本信息
 */
...
/**
Quartz提供的锁表，
为多个节点调度提供分布式锁，实现分布式调度，默认有2个锁

STATE_ACCESS主要用在scheduler定期检查是否失效的时候，
保证只有一个节点去处理已经失效的scheduler；

TRIGGER_ACCESS主要用在TRIGGER被调度的时候，
保证只有一个节点去执行调度；
 */
